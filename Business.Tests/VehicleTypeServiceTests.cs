...
public class VehicleTypeServiceTests
{
	...
	{
		// Arrange
		A.CallTo(() => _fakeDbService.GetAsync<SimulatedVehicleApiData, SimulatedVehicleApiDataDTOPlateAndType>
			(A<Expression<Func<SimulatedVehicleApiData, bool>>>._)
		)
		.ReturnsLazily(call => Task.FromResult(_fakeSimulatedVehicleApiData
			.Where(dto => _fakeTollPassageData.Select(p => p.PlateNumber)

...
public class TollCameraService(IDbService _dbService) : ITollCameraService
{
	...
	public async Task<List<TollCameraData>> GetDailyTollCameraData(DateTime date, int numberOfPassages)
	{
		...
		var simulatedPlateNumberScans = await _dbService.GetAsync<SimulatedVehicleApiData, SimulatedVehicleApiDataPlateNumber>();

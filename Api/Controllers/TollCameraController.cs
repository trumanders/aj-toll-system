...
[ApiController]
public class TollCameraController(ITollCameraService _tollCameraService, ITollFreeDaysService _tollFreeDaysService) : ControllerBase
{
	...
	public async Task<IResult> GetDailyTollCameraData([FromQuery]DateTime date, [FromQuery]int numberOfPassages)
	{
		...
		try
		{
			...
			var dtos = dailyTollCameraData.Select(data => new TollCameraDataDTO
			{
